...
const decodeError = () => Object.assign(new EventTarget(), {
    duration: NaN,
    currentTime: NaN,
    error: {
...
export const AMRPlayer = (file, { onEnd = () => { } } = {}) => {
    ...
    const createBufferSource = (startFrom = 0) => {
        ...
        if (offset >= samplesLen)
            offset = samplesLen - 8000;
        ...
        bufferSource.theEnd = new Promise(resolve => {
    ...
    const play = async () => {
        if (isPlaying || !bufferSource)
            return;
        if (pauseOffset >= duration * 1000)
            return stop();
